var cbWSClient = function(url) {
  ...
  this.doDescribe = function(module) {
    ...
    // reqtype = 'GET';
    let params =
      "?operation=describe&sessionName=" + this._sessionid + "&elementType=" + module;
    this.fetchOptions.method = "get";
    delete this.fetchOptions.body;
    let myself = this;
    return fetch(this._serviceurl + params, this.fetchOptions)
  ...
  this.doGetRelatedRecords = function(record, module, relatedModule, queryParameters) {
    ...
    // reqtype = 'POST';
    let postdata =
      "operation=getRelatedRecords&sessionName=" +
      this._sessionid +
      "&id=" +
      record +
      "&module=" +
  ...
  this.doSetRelated = function(relate_this_id, with_these_ids) {
    ...
    // reqtype = 'POST';
    let postdata =
      "operation=SetRelation&sessionName=" +
      this._sessionid +
      "&relate_this_id=" +
      relate_this_id +
      "&with_these_ids=" +
      JSON.stringify(with_these_ids);
    this.fetchOptions.body = postdata;
    this.fetchOptions.method = "post";
    let myself = this;
    return new Promise((resolve, reject) => {
      fetch(this._serviceurl, this.fetchOptions)
        .then(this.status)
        .then(this.getData)
        .then(data => {
          if (myself.hasError(data) == false) {
            ...
            return reject(new Error("incorrect response: " + myself.lastError()));
          }
        })
        .catch(error => reject(error));
    });
